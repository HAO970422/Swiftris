...
class Shape: Hashable, CustomStringConvertible
{
    ...
    // a final funciton cannot be overrided by subclasses
    final func initializeBlocks()
    {
        guard let blockRowColumnTranslations = blockRowColumnPositions[orientation]
        else
        {
            return
        }
        ...
        {
            (diff) -> Block in return Block(column: column + diff.columnDiff, row : row + diff.rowDiff, color: color)
        }
    }
